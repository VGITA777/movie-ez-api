package com.prince.movieezapi.media.api.models.tvseries;


/**
 * Response model for The Movie Database (TMDB) API's TV series latest endpoint.
 * Returns the most recently created TV series. This is live data and will continuously change.
 *
 * @see <a href="https://developer.themoviedb.org/reference/tv-series-latest-id">Tv Series Latest API Reference</a>
 */
public class TvSeriesLatestModel extends TvSeriesDetailsModel {
}
